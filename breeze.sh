...
myread_dig pick
    ...
    ;;
    4) #Описание теста производительности
		echo 'Для теста скорости диска мы пытаемся записать на диск кусок в 64Кб
		16 тысяч раз (общий объём данных 1000 Мб). Тест прогоняем трижды,
		показываем каждый результат по отдельности, а также среднее значение.
		Заодно вы сможете оценить насколько сильно "плавает" это значение от
		одного прохода к другому.'
		wait
    ;;
    0)
     chosen=0
