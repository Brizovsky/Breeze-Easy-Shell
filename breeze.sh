...
{
if [ $1 == "UTC" ]
	then /bin/cp /usr/share/zoneinfo/$1  /etc/localtime
	else /bin/cp /usr/share/zoneinfo/$1/$2  /etc/localtime
fi
br
...
repo() {
	...
	if [ $osver1_repo -eq 0 ]; then
	    echo "Не получилось определить версию Вашей ОС, но Вы можете выбрать её сами на свой страх и риск:"
...
if [ $osver1 -eq 7 ]; then
	...
	# переключим с firewalld на iptables, иначе после ребута может стартануть firewalld, iptables не включится,
	# а раз уж мы сохраняем правила для ребута, то мы этого не хотим.
...
case "$1" in
	...
	disk_result=$(df -l -T | grep -E 'ext|xfs' | awk '{print $'$n'}' | ( #Вариант, который берет только с файловыми системами extX
		sum=0 #обнуляем сумму
		while read hdd #начинаем перебор, пока есть строки (на случай, если несколько винтов)
		do
			sum=$(($sum+$hdd)) #прибавляем к сумме значения каждого следующего диска
		done
		echo $sum #выплевываем значение, чтобы записать его в предыдущую переменную
		)
	)
}
...
│ ├───┼─────────────────────────────────────────────────┤
├─┤ 2 │ Удалить ВСЕ лог-файлы (*.log)                   │
│ ├───┼─────────────────────────────────────────────────┤
├─┤ 3 │ Удалить логи Apache, Nginx, Squid и прочие логи │
│ ├───┼─────────────────────────────────────────────────┤
├─┤ 4 │ Удалить логи Apache конкретного пользователя    │
│ ├───┼─────────────────────────────────────────────────┤
├─┤ 5 │ Посмотреть сколько свободного места на диске    │
│ ├───┼─────────────────────────────────────────────────┤
└─┤ 0 │ Выйти на уровень вверх                          │
  └───┴─────────────────────────────────────────────────┘
"
...
END
		    ...
		    echo "IP: $ipext"
		    echo "Порт: $port"
			echo -n "Пользователь: $login"
		    if [[ $ans = 2 ]]; then #если без авторизации, то логин не выводится, заменяем заглшкой
		    	echo "[без авторизации]"
			else
				br #ликвидируем ключ -n в случае если он нам не нужен
		    fi
		    br
		    wait
		;;
		2) #Удалить Dante
			my_clear
			echo "Начинаем удаление Dante..."
			uninstall dante-server
			rm -r /var/run/sockd -f
			br
...
myread_dig pick
    ...
    ;;
    2) #Удалить ВСЕ лог-файлы (*.log)
	echo "Внимание! Будут удалены абсолютно все файлы с диска, подходящие по маске *.log"
	echo "Продолжить?"
	myread_yn ans
	case "$ans" in
		y|Y)
		disk used
		usedspace1=$disk_result #запоминаем сколько было занято места до очистки
		echo "Начинаем поиск и удаление лог-файлов..."
		find / -type f -name "*.log" -delete
		disk used
		usedspace2=$disk_result #смотрим сколько занято теперь
