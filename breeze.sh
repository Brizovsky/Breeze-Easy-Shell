...
myread_dig pick
    ...
    2) #Провести тест скорости CPU
		...
		if [ $cpu_cores -gt 1 ]; then echo "В пересчете на одно ядро мощность вашего ядра составляет $powerpercorepercent% от эталонного." #пересчет на 1 ядро выводим только если ядер больше одного
		br
		wait
    ;;
    3) #Провести тест скорости диска
