...
myread_dig pick
	...
	case "$ans" in
		y|Y)
		usedspace1=`df | grep "/dev/" | sed q |  awk {'print $3'}` #запоминаем сколько было занято места до очистки
		echo "Начинаем поиск и удаление лог-файлов..."
		find / -type f -name "*.log" -delete
		usedspace2=`df | grep "/dev/" | sed q |  awk {'print $3'}` #смотрим сколько занято теперь
